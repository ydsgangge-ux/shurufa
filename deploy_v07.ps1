# Deploy v0.7
# Files changed: ai_ime_ime.py, user_memory.py, candidates.py, dict_loader.py
...
Write-Host "=== Deploy v0.7 ===" -ForegroundColor Cyan

# Remove nested ai_ime\ai_ime\ if exists
$nestedDir = Join-Path $dst "ai_ime"
if (Test-Path $nestedDir) {
    Remove-Item $nestedDir -Recurse -Force
    Write-Host "  Removed nested dir." -ForegroundColor Green
}
...
Write-Host ""
Write-Host "=== Deploy v0.7 Complete ===" -ForegroundColor Green
Write-Host "Test in Notepad:"
Write-Host "  songhuodan -> page to find greedy result"
Write-Host "  select it -> next time songhuodan or shd will find it"
Write-Host "  Shift -> toggle Chinese/English"
